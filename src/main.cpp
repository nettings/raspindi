...
static void event_loop(RPiCamEncoder &app)
{
	...
	std::unique_ptr<Output> output = std::unique_ptr<Output>(
		new NdiOutput(options, _getValue("neopixel_path", "/tmp/neopixel.state"), _getValue("ndi_source_name", "Video Feed"))
	);
